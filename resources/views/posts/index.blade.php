...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        @if ($recommendation && !empty($recommendation->recommended_items['ideas_nuevos_posts']))
            <div class="card shadow border-0 mb-4 position-fixed top-50 start-50 translate-middle" 
                ...
                <div class="card-body p-4">
                    ...
                    <div id="recommendationCarousel" class="position-relative">
                        @foreach ($recommendation->recommended_items['ideas_nuevos_posts'] as $index => $idea)
                            <div class="recommendation-item {{ $index === 0 ? 'active' : '' }}" 
                                 data-index="{{ $index }}">
                                <div class="bg-light rounded p-3 mb-3">
                                    <h6 class="text-dark fw-bold mb-2">
                                        <i class="bi bi-stars text-warning me-1"></i>{{ $idea['titulo'] }}
                                    </h6>
                                    <p class="text-muted mb-0 small">{{ $idea['descripcion'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
                        ...
                        <div class="d-flex gap-2">
                            @foreach ($recommendation->recommended_items['ideas_nuevos_posts'] as $index => $idea)
                                <span class="carousel-indicator {{ $index === 0 ? 'active' : '' }}" 
                                      data-index="{{ $index }}"
                                      style="width: 8px; height: 8px; border-radius: 50%; background: {{ $index === 0 ? '#0d6efd' : '#dee2e6' }}; cursor: pointer;"
                                      onclick="goToSlide({{ $index }})"></span>
                            @endforeach
                        </div>
                        <small class="text-muted">
                            <span id="currentSlide">1</span> / {{ count($recommendation->recommended_items['ideas_nuevos_posts']) }}
                        </small>
                    </div>
                </div>
            </div>
    ...
    @forelse ($posts as $post)
        <div class="col">
            <div class="card h-100 shadow-sm border-0 hover-lift">
                <a href="{{ route('posts.show', $post->id) }}" class="text-decoration-none text-dark d-block">
                    @if ($post->image)
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="card-text text-muted small flex-grow-1">{{ Str::limit($post->content, 100) }}</p>
                ...
                <div class="px-3 py-2 border-top">
                    <button 
                        ...
                        <i id="heart-icon-{{ $post->id }}" 
                           class="bi {{ $post->isLikedBy(Auth::user()) ? 'bi-heart-fill' : 'bi-heart' }}"
                           style="color: {{ $post->isLikedBy(Auth::user()) ? '#dc3545' : '#6c757d' }}; transition: all 0.2s;"></i>
                    </button>
                    <span id="like-count-{{ $post->id }}" class="ms-1 text-muted small fw-bold">
                        {{ $post->likeCount() }}
                    </span>
                </div>
                ...
                <div class="card-footer bg-white border-top-0 d-flex gap-2 pt-0 pb-3">
                    ...
                    @else
                        <a href="{{ route('posts.show', $post->id) }}" class="btn btn-outline-secondary btn-sm w-100">
